import os, re, math, time, logging, unicodedata, csv, requests
from datetime import datetime, timedelta
from typing import Dict, Optional, Tuple, List

import numpy as np
import pandas as pd
import requests as _req_for_scraper
...
def backoff_get(url: str, max_tries: int = 5, sleep_base: float = 1.0) -> _req_for_scraper.Response:
    last_exc = None
    for i in range(max_tries):
        try:
            r = _req_for_scraper.get(url, headers=HEADERS, timeout=30)
            if r.status_code == 200:
                return r
            else:
                log.warning(f"GET {url} -> {r.status_code}")
        except _req_for_scraper.RequestException as e:
            last_exc = e
...
# --- Descarga automática del modelo por MODEL_URL ---
def ensure_model_available() -> str:
    ...
    with _req_for_scraper.get(url, stream=True, timeout=300) as r:
        r.raise_for_status()
...
# --- Lookup por CSV + fallback a comunas ---
