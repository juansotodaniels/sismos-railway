...
# - Parser robusto del "Reporte de intensidades" (2–4 columnas, rangos III–IV, fallback <li>)
...
# - Fechas en TZ America/Santiago; override con FECHA_ACTUAL=YYYY-MM-DD y DESDE_FECHA=YYYY-MM-DD
...
def ensure_model_available() -> str:
    path = os.getenv("MODEL_PATH", "modelos/Sismos_17-01-2025.pkl")  # Railway: puedes setear /data/archivo.pkl
...
# ======== NUEVO: parser robusto de intensidades ========
...
def parse_intensidades(soup: BeautifulSoup):
    """Devuelve lista de (comuna, intensidad_int) desde el reporte.
       Soporta tablas de 2-4 columnas y listas <li>."""
    ...
    # 2) Fallback: listas <li> tipo "Localidad: IV" o "Localidad – III-IV"
    if not resultados:
        ...
        for a in items:
            try:
                ...
                for comuna, y in pares:
                    ...
                    ypred = int(np.round(joblib.Parallel(n_jobs=1, prefer="threads")(delayed=lambda m, X: m.predict(X))(modelo, X))[0]) if False else int(np.round(modelo.predict(X))[0])
...
if __name__ == "__main__":
    desde = os.getenv("DESDE_FECHA")        # YYYY-MM-DD
    fecha_actual_override = os.getenv("FECHA_ACTUAL")  # YYYY-MM-DD
