# app.py
# Scraper de sismologia.cl (percibidos) sin Selenium, listo para Railway.
# - Parser robusto CSN (tablas 2–4 columnas, rangos III–IV, fallback <li>)
# - Fallback SENAPRED: senapred.cl (SPA) -> web.senapred.cl (SSR) + parser propio
# - Limpieza numérica (°, km, comas), fechas TZ America/Santiago
# - Inserción atómica con SQLAlchemy
...
HEADERS = {
    "User-Agent": "Mozilla/5.0 (compatible; SismosBot/1.0; +https://example.com/bot)",
...
def ensure_model_available() -> str:
    path = os.getenv("MODEL_PATH", "modelos/Sismos_17-01-2025.pkl")  # Puedes usar /data/archivo.pkl en Railway
...
# ======== NUEVO: helpers SENAPRED ========

def is_senapred_spa_html(text: str) -> bool:
    return "You need to enable JavaScript to run this app." in (text or "")

def senapred_to_web(url: str) -> str:
    """senapred.cl → web.senapred.cl (misma ruta)"""
    try:
        u = urlparse(url)
        if u.netloc == "senapred.cl":
...
def _roman_value_from_text(txt: str):
    """Devuelve el entero de la(s) cifra(s) romana(s) en el texto.
       Si hay rango (III–IV / III a IV), devuelve el MÁXIMO."""
    if not txt:
        return None
    tokens = re.findall(r'\b[IVXLCDM]{1,4}\b', txt.upper())
    vals = [ROMANOS[t] for t in tokens if t in ROMANOS]
    return max(vals) if vals else None

def _is_header_cell(s: str) -> bool:
    s = (s or "").strip().lower()
    return any(k in s for k in ("región", "region", "localidad", "comuna", "intensidad"))

def parse_intensidades_senapred(soup: BeautifulSoup):
    """
    Extrae [(localidad, intensidad_int)] desde páginas de SENAPRED (web.senapred.cl)
    con texto tipo 'Iquique: IV', 'Alto Hospicio: III', o rangos 'III–IV'.
    """
    resultados: List[Tuple[str,int]] = []
    cont = soup.select_one("article") or soup.select_one(".entry-content") or soup
    text = cont.get_text("\n", strip=True) if cont else soup.get_text("\n", strip=True)

    gatillos = ["Las intensidades en escala de Mercalli", "Las intensidades en Escala de Mercalli"]
    for g in gatillos:
        if g in text:
            text = text.split(g, 1)[1]
            break
    ...
    for line in text.splitlines():
        ...
        if not line:
            continue
        if re.match(r"^regi[oó]n\s*:", line.lower()):
            continue
        ...
        loc = m.group(1).strip()
        rhs = m.group(2).strip()
        val = _roman_value_from_text(rhs)
        if loc and val:
            resultados.append((loc, int(val)))

    return resultados

# ======== Parser CSN (tablas y <li>) ========

def parse_intensidades(soup: BeautifulSoup):
    """Devuelve lista de (comuna, intensidad_int) desde CSN.
       Soporta tablas 2–4 columnas y listas <li>."""
    resultados: List[Tuple[str,int]] = []
    cont = soup.select_one("#publishView") or soup

    # Tablas
    tablas = cont.select("table")
    for table in tablas:
        for tr in table.select("tr"):
            tds = [td.get_text(" ", strip=True) for td in tr.select("td")]
            if not tds:
                continue
            if any(_is_header_cell(x) for x in tds):
                continue

            inten = None
            inten_idx = -1
            for idx, cell in enumerate(tds):
                rv = _roman_value_from_text(cell)
                if rv:
                    inten = rv
                    inten_idx = idx
                    break
            ...
            for idx, cell in enumerate(tds):
                if idx == inten_idx:
                    continue
                if not _is_header_cell(cell) and _roman_value_from_text(cell) is None:
                    localidad = cell.replace(":", "").strip()
                    break
            if not localidad and len(tds) == 2:
                localidad = tds[0].replace(":", "").strip()

            if localidad and inten:
                resultados.append((localidad, int(inten)))
    ...
    if not resultados:
        for li in cont.select("li"):
            ...
            if m:
                loc = m.group(1).strip()
                inten = _roman_value_from_text(m.group(2))
                if loc and inten:
                    resultados.append((loc, int(inten)))
    ...
    for i in range(dias + 1):
        ...
        for a in items:
            try:
                ...
                magnitud = float(str(mag_raw).replace(",", ".")) if mag_raw is not None else 0.0
                ...
                # Fallback SENAPRED SPA → web.senapred.cl
                orig_rep_link = rep_link
                if "senapred.cl" in rep_link and "web.senapred.cl" not in rep_link:
                    rep_link = senapred_to_web(rep_link)

                log.info(f"[SISMO {id_sismo}] reporte: {rep_link}")

                # Descarga reporte
                rep_resp = backoff_get(rep_link)
                rep_html = rep_resp.text or ""

                # Si es SPA (placeholder JS), intenta forzar dominio web.senapred.cl
                if is_senapred_spa_html(rep_html) and "senapred.cl" in orig_rep_link:
                    alt = senapred_to_web(orig_rep_link)
                    if alt != rep_link:
                        try:
                            log.info(f"[SISMO {id_sismo}] SPA detectado; reintentando con {alt}")
                            rep_resp = backoff_get(alt)
                            rep_html = rep_resp.text or ""
                            rep_link = alt
                        except Exception as e:
                            log.warning(f"[SISMO {id_sismo}] Fallback SENAPRED falló: {e}")

                rep_soup = BeautifulSoup(rep_html, "lxml")

                # Parser según dominio
                if "web.senapred.cl" in rep_link:
                    pares = parse_intensidades_senapred(rep_soup)
                else:
                    pares = parse_intensidades(rep_soup)  # parser CSN original
...
if __name__ == "__main__":
    desde = os.getenv("DESDE_FECHA")               # YYYY-MM-DD (opcional)
    fecha_actual_override = os.getenv("FECHA_ACTUAL")  # YYYY-MM-DD (opcional)
